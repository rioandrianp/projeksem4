...
<body>
  ...
  <main id="main" class="main">
    <div class="container mt-5">
        <h1 class="mb-4">Form Tarik Saldo</h1>
        @if (session('success'))
            <div class="alert alert-success">{{ session('success') }}</div>
        @endif
        <div class="mb-3">
            <p>Saldo Anda saat ini: {{ number_format($saldo, 0, ',', '.') }}</p>
        </div>
        <form action="{{ route('tarik-saldo.store') }}" method="POST">
            @csrf
            <div class="mb-3">
                <label for="jumlah" class="form-label">Jumlah Tarik Saldo</label>
                <input type="number" class="form-control" id="jumlah" name="jumlah" required
                       max="{{ $saldo }}" placeholder="Max: {{ number_format($saldo, 0, ',', '.') }}">
            </div>
